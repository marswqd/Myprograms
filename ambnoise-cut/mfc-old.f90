program mfc
 implicit none
!-----
! 此程序用于多重滤波提取基阶面波相速度频散信息(只接受一个搜索点)
! 作者：王清东，武大09研，固体地球物理学
! 时间：2012-06-08 10:36:37
!
! 部分参数说明：
! ntc     I*4  窄带滤波周期数
...
!              第二行为搜索的速度范围 vlow,vhigh
!              第三行为滤波起始周期,截止周期,周期间隔 btc,etc,dtc
!              第四行为周期范围两端的多重滤波的alpha值 alpha1,alpha2
!              自第五行一下为输入的文件名 sacname
!
!
! 部分子程序说明：
...
!-----
 integer*4::i,j,num,NPTS,VNPTS,PNPTS,nerr,ntc,pl,ph,vl,vh,tcb,ttb,bbt,interp
 real*4::t0,v0,btc,dtc,etc,dy1,dyn,a,b,alpha1,alpha2,alpha,O,vlow,vhigh,minhf0,minhf
 real*4,allocatable::tc(:),tt(:),wv(:),tt1(:),temp1(:),temp2(:)
 real*4,allocatable::sT_t(:,:),sT_C(:,:),TttC(:,:),xy(:,:)
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 do i=1,num
  ...
  vl=1
  VNPTS=vh-vl+1
  ph=anint((rhdr(51)/vlow-rhdr(6)+O)/rhdr(1))+1     !确定绘图窗
...
!搜索:走时-周期矩阵;(相速度-周期矩阵)
  call seek(sT_t,PNPTS,ntc,tcb,ttb,nerr)
  TttC(tcb,1)=tc(tcb)
  TttC(tcb,2)=tt(ttb)
  TttC(tcb,3)=rhdr(51)/tt(ttb)
  TttC(tcb,4)=rhdr(51)/(tt(ttb)+tc(tcb)/8)
  TttC(tcb,5)=rhdr(51)/tt(ttb)-rhdr(51)/(tt(ttb)+tc(tcb)/8)
  bbt=ttb
  do j=tcb-1,1,-1
...
!图示化:周期-速度图
  xy(:,1)=TttC(:,1)
  xy(:,2)=TttC(:,3)
  call draw(sT_C,ntc,PNPTS,xy,trim(sacname(i)),t0,v0)
...
!yb:输入输出
 ...
 if(yb.eq.1.or.yb.eq.m) then
  print*, 'Search out of range, please check, please check'
  nerr=1
  return
 endif
 do i=yb,2,-1
...
!图示化功能实现,需要CPS3.30的库文件libcalpltf.a
 ...
 enddo
 call putxy(xy,m,2,firstx,deltax,firsty,deltay,t0,v0)
