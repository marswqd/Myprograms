!功能: 带通滤波
!
!作者: 王清东 时间: 2013/3/21 20:34:44
program bpf
 implicit none
 integer*4::i,j,NPTS,num,nerr,nn,head,n
 real*4::T1,T2,f1,f2,f3,f4,DT,B,E,P,S,NB,NE,a(4)
 character::wname*60,cmd*200
...
!读取控制文件信息
 open(11,file='bpf.in',status='old',action='read')
 read(11,*) num,f1,f2,f3,f4
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 do i=1,num
  ...
  ! a(4)=E
  if(P/=-12345.0e0) then
   if(P<B+3.0e0.or.P>E-3.0e0) then
    P=-12345.0e0
   else
    n=nint((P-B)/DT)+1
    do j=n-2,n+2
     if(data(j)==0.0e0) then
      P=-12345.0e0
      exit
     endif
    enddo
  ...
  if(P/=-12345.0e0) then
   if(B>P-5.0e0) a(1)=P-5.0e0
   if(E<P+5.0e0) a(2)=P+5.0e0
  endif
  if(S/=-12345.0e0) then
   if(S<B+3.0e0.or.S>E-3.0e0) then
    S=-12345.0e0
   else
    n=nint((S-B)/DT)+1
    do j=n-2,n+2
  ...
  if(S/=-12345.0e0) then
   if(B>S-5.0e0) a(3)=S-5.0e0
   if(E<S+5.0e0) a(4)=S+5.0e0
  endif
  if(P==-12345.0e0.and.S==-12345.0e0) then
   cmd='mv '//trim(sacname(i))//' del/'
   call system(cmd,nerr)        !将无标定文件移到文件夹del中
   write(10,'("No-phase file, delete ",a)') trim(sacname(i))
   write(*,'("No-phase file, delete ",a)') trim(sacname(i))
   deallocate(data)
   cycle
  endif
  NB=minval(a)
  ...
  adata=0.0e0
  head=nint((B-NB)/DT)+1
  adata(head:head+NPTS-1)=data(1:NPTS)
  !call bpfn(data,NPTS,DT,1/f4,1/f3,1/f2,1/f1)
  rhdr(6)=NB
  rhdr(7)=NE
  rhdr(9)=P
  rhdr(11)=S
  ihdr(10)=nn
  call bpfn(adata,nn,DT,1/f4,1/f3,1/f2,1/f1)
  wname='P.'//trim(sacname(i))
  print*,wname
  !call wsac(data,NPTS,wname)
  call wsac(adata,nn,wname)
  deallocate(data,adata)
  cmd='mv '//trim(sacname(i))//' ../'
  call system(cmd,nerr)
 enddo
