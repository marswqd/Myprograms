function [st,t,f] = GST(timeseries,lan,p,minfreq,maxfreq,samplingrate,freqsamplingrate)
...
if nargin == 0 
   if verbose 
       disp('No parameters inputted.'),end
   st_help
   t = 0; st = -1; f = 0; 
   lan = 1; p = 1;
   return
...
elseif nargin == 7
   ...
   if verbose 
       disp('Error in input arguments: using defaults'),end
   minfreq = 0;
   maxfreq = fix(length(timeseries)/2);
   samplingrate = 1;
   freqsamplingrate = 1;
...
if verbose 
   fprintf('Minfreq = %d',minfreq)
   fprintf('Maxfreq = %d',maxfreq)
   fprintf('Sampling Rate (time   domain) = %d',samplingrate)
   fprintf('Sampling Rate (freq.  domain) = %d',freqsamplingrate)
   fprintf('The length of the timeseries is %d points',length(timeseries))
   disp(' ')
end
...
if verbose 
    fprintf('The number of frequency voices is %d',spe_nelements),end


% The actual S Transform function is here:
st = strans(timeseries,lan,p,minfreq,maxfreq,samplingrate,freqsamplingrate,verbose,removeedge,analytic_signal,factor); 
...
if nargout == 0 
   if verbose 
       disp('Plotting pseudocolor image'),end
   pcolor(t,f,abs(st))
end
...
function st = strans(timeseries,lan,p,minfreq,maxfreq,samplingrate,freqsamplingrate,verbose,removeedge,analytic_signal,factor) 
...
if removeedge
    if verbose 
        disp('Removing trend with polynomial fit'),end
 	 ind = (0:n-1)';
    r = polyfit(ind,timeseries,2);
    fit = polyval(r,ind) ;
	 timeseries = timeseries - fit;
    if verbose 
        disp('Removing edges with 5% hanning taper'),end
...
if analytic_signal
   if verbose 
       disp('Calculating analytic signal (using Hilbert transform)'),end
...
if verbose 
    fprintf('Estimated time is %f',tim_est),end
...
if verbose 
    disp('Calculating S transform...'),end
if minfreq == 0
    st(1,:) = mean(timeseries)*(1&(1:1:n));
else
  	st(1,:) = ifft(vector_fft(minfreq+1:minfreq+n).*g_window(n,minfreq,factor,lan,p));
end
...
for banana = freqsamplingrate:freqsamplingrate:(maxfreq-minfreq)
   st(banana/freqsamplingrate+1,:) = ifft(vector_fft(minfreq+banana+1:minfreq+banana+n).*g_window(n,minfreq+banana,factor,lan,p));
end   % a fruit loop!   aaaaa ha ha ha ha ha ha ha ha ha ha
% End loop to increment the frequency point
if verbose 
    disp('Finished Calculation'),end

%%% end strans function

%------------------------------------------------------------------------
function gauss = g_window(length,freq,factor,lan,p)
...
if l > 1  
   if verbose 
       disp('Array of inputs accepted.'),end
   temp = minfreq;
   minfreq = temp(1);
   if l > 1  
       maxfreq = temp(2);end;
   ...
   if l > 4  
      if verbose 
          disp('Ignoring extra input parameters.'),end
   end;
   ...
   if maxfreq > length(timeseries)/2  || maxfreq < 0 
      maxfreq = fix(length(timeseries)/2);
      if verbose 
          fprintf('Maxfreq < 0 or > Nyquist. Setting maxfreq = %d',maxfreq),end
   end
   if minfreq > maxfreq 
      temporary = minfreq;
      minfreq = maxfreq;
      ...
      clear temporary;
      if verbose 
          disp('Swapping maxfreq <=> minfreq.'),end
   end
   if samplingrate <0
      samplingrate = abs(samplingrate);
      if verbose 
          disp('Samplingrate <0. Setting samplingrate to its absolute value.'),end
   end
   if freqsamplingrate < 0   % check 'what if freqsamplingrate > maxfreq - minfreq' case
      freqsamplingrate = abs(freqsamplingrate);
      if verbose 
          disp('Frequency Samplingrate negative, taking absolute value'),end
   end
