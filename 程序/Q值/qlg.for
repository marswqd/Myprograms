...
c      logical verbose             ! verbose or quite mode
...
c      logical checkerboard        ! set true for checker test
      real cbdelta                ! delta +/- apriror value
      parameter (cbdelta=0.0004)
...
c      real f_low,f_high                 ! filters
      character*1 option,ch    
      character*80 outfile
      integer write1
c      character*1 orientation           ! sensor orientation, v or h 
...
c
     ...
     &                sel_stat_list(j,2).eq.wav_comp(i)) then
                     tflag=.true.
...
c
                ...
                write(61,'(f7.3,1x,f8.3,1x,a7,1x,a5)') 
     &                  slat,slon,'5 0 1 7',wav_stat(i)
              endif
...
c
c              if (level_p(f_choice).gt.0..or.
c     &         level_p(f_choice).gt.2.*level_n(f_choice)) then
c
...
c
         ...
         do i=1,ygrid_npts-1
           do j=1,xgrid_npts-1
             b=b+1
             write(22,*) grid_xy(b,1),grid_xy(b,2),alpha_apriori+m(b)
           enddo
         enddo
...
c      integer max,xmax,ymax
      ...
      integer xint,yint
      real b,d1,d2,m,x,y,x1,x2,y1,y2,dist,dedeg,az0,np
      integer flag,ind
      real pi
      parameter (pi=3.141593)
      logical done
...
c
      ...
      elseif (line(1:12).eq.'CHECKERBOARD') then
        read(line(41:50),*) var
