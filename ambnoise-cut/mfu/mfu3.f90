...
!-----
 ...
 real*4::alpha,t0,v0,btc,dtc,etc,dy1,dyn,ttc,O
 real*4,allocatable::tc(:),tt(:),wv(:),tt1(:),temp1(:),temp2(:)
 real*4,allocatable::mT_t(:,:),mT_U(:,:),TttU(:,:),xy(:,:)
...
!读取控制文件信息
 open(11,file='mfufile',status='old')
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 do i=1,num
  ...
  allocate(mT_t(0:PNPTS,0:ntc),mT_U(0:PNPTS,0:ntc),TttU(ntc,5),xy(ntc,2))
...
!图示化:周期-速度图
  xy(:,1)=TttU(:,1)
  xy(:,2)=TttU(:,3)
  name=trim(sacname(i))//'.U'
  call draw(mT_U,ntc,PNPTS,xy,trim(name))

  deallocate(mT_t,mT_U,tt,wv,tt1,TttU,xy)
 enddo
