...
#存储速度模型（二进制）
#taup_create.bat -nd $mod #-verbose
#taup_time.bat -km 4.741 -h 11.000 -time -mod $mod | awk '{print $1}'
taup_time.bat -km 200 -h 11.000 -mod $mod -time -ph Pn
taup_time.bat -km 200 -h 11.000 -mod $mod -time -ph p
taup_time.bat -km 200 -h 11.000 -mod $mod
time96 -DIST 200 -EVDP 11.0 -P -T -M yl.mod
#echo $a
# rm $out
# touch $out
...
# for((i=0;i<n;i++));do
 ...
 # if [ ${p[i]} = P ];then
  ...
  # caltt=`taup_time.bat -km ${dist[i]} -h ${evdp[i]} -time -mod $mod | awk '{print $1}'`
 # else
