...
!读取控制文件信息 
 open(11,file='btsfile',status='old')
 !tn=ob:onebit
 !tn=rm:runmean
 !sw=no:不做谱白化
 !sw=on:做谱白化
 read(11,*) num                          !num:处理的sac文件数
 do i=1,3                                !下面3行的内容是bp tn 和 sw
  read(11,*) flag
  if(flag.eq.'bp'.or.flag.eq.'BP') then
   ...
   bt4=1.1e0*bt3
  else if(flag.eq.'tn'.or.flag.eq.'TN') then
   backspace(11)
   read(11,*) flag,tn
   if(tn.eq.'rm'.or.tn.eq.'RM') then
    backspace(11)
    read(11,*) flag,tn,wn,t2,t3          !选择:wn=50 t2=7.0 t3=45.0
    t1=0.9e0*t2
    t4=1.1e0*t3
   endif
  else if(flag.eq.'sw'.or.flag.eq.'SW') then
   backspace(11)
   read(11,*) flag,sw
   if(sw.eq.'on'.or.sw.eq.'ON') then
    backspace(11)
    read(11,*) flag,sw,wns               !选择:wn=50
   endif
  else
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)
  ...
  if(rhdr(2).eq.0.0e0.and.rhdr(3).eq.0.0e0) then
   write(10,*) 'Zero file : ', trim(sacname(i))
   goto 98
  endif
!按地震仪频带宽度进行带通滤波,去处干扰
  call bpf(dfft,nfft,logn,rhdr(1),bt1,bt2,bt3,bt4)
  if(tn.eq.'ob'.or.tn.eq.'OB') then
!onebit,去除地震
   call onebit(dfft,nfft)                    !时域归一化:onebit
  else if(tn.eq.'rm'.or.tn.eq.'RM') then
!滑动绝对平均,去除地震
   call runmean(dfft,nfft,logn,rhdr(1),wn,t1,t2,t3,t4)  !时域归一化:滑动绝对平均法
  endif
!谱白化
  if(sw.eq.'on'.or.sw.eq.'ON') then
   call srunmean(dfft,nfft,logn,wns,nerr)
  endif
