!功能: 计算两波形数据震相倒时差,生成初始互相关走时差文件:mcd.txt,用于ccme.f90
!  注: 两波形数据必须是同一台站记录到的(数据采样间隔一致)
!      处理的SAC文件命名形式为:P.BAS.20120907121632.Z.SAC
!      滤波+按震相对齐+单滑动窗双滑动量+时域多重互相关检测器(detector)计算时差
!      (单滑动时窗宽度:-0.5-1s,双滑动量滑动量:1.5s and 2s)
...
!      mcd.in中输入Z分量文件名; epair为要计算的地震事件对的id,每行一个事件对
!      ZR分量计算P震相走时差,RT分量计算S震相走时差; 时差为正表示数据1的走时大::tt1-tt2
!作者: 王清东 时间: 2015/8/24 21:04:07
program mcd
 implicit none
...
!读取控制文件信息
 ...
 read(11,'(a)') epair       !地震事件对文件:   # id1 id2 dist12
 read(11,'(a100)') card
 read(card,*) f0
 if(f0==1) read(card,*) f0,f1,f2,f3,f4 !f0=1:带通滤波(Hz),1-2-6-8
 ! iphase=1 P only ; iphase=2 S only ; iphase=else P and S
 read(11,*) iphase,pwl,pwr,swl,swr !P S 初始时窗位置设定,震相到时左右偏移量(s)
 ! 一般 pwl=-0.5e0 pwr=1.0e0 swl=-0.5e0 swr=1.0e0
 ! slidewin=2 双滑动窗 ; slidewin=else 单滑动窗
 read(11,*) slidewin,sld1,sld2   !滑动量,一般sld1=1.5e0 sld2=2.0e0
 read(11,*) maxdist,maxsep,ccut  !台站到震源最大距离250km;最大震源分离距离10km;相关系数截止值0.5;
 !read(11,*) num
 !allocate(sacname(num),eve(num))
 read(11,'(a)') flist     !存储要处理的波形文件名
 close(11)
 ...
 !open(12,file='mcd.txt')   !初始互相关走时差文件
...
!找到SAC文件所包含的事件数和其在sacname数组中的位置
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 ...
 do i=1,nep
  ...
  do i1=1,sta(ii)
   ...
   do i2=1,sta(jj)
    j2=pos(i2,jj)
    name2=sacname(j2)    !不同事件的相同台站.分量记录
    if(name1(mm+1:nn-1)==name2(mm+1:nn-1).and.name1(kk+1:kk+1)==name2(kk+1:kk+1)) then
     ...
     if(iphase/=2.and.P1/=-12345.0e0.and.P2/=-12345.0e0) then
      ...
      !主文件
      wln1=nint((P1+pwl-B1)/DT)+1  !确定初始时窗:P
      wrn1=nint((P1+pwr-B1)/DT)+1
      !wln11=nint((P1+swl-B1)/DT)+1  !第二初始时窗:P
      !wrn11=nint((P1+swr-B1)/DT)+1
      wn=abs(wrn1-wln1+1)
      !wnn=abs(wrn11-wln11+1)
      if(slidewin==2) then !主文件和副文件都设置滑动窗
       sln1=nint((P1+pwl-slidel1-B1)/DT)+1
       srn1=nint((P1+pwr+slider1-B1)/DT)+1  !第一滑动窗边界
       sln11=nint((P1+pwl-slidel2-B1)/DT)+1
       srn11=nint((P1+pwr+slider2-B1)/DT)+1  !第二滑动窗边界
      else
       sln1=wln1
       srn1=wrn1
       sln11=wln1
      ...
      if(srn1>NPTS1) srn1=NPTS1        !判断边界是否超出数据范围
      if(sln1<1) sln1=1
     ...
     if(iphase/=1.and.S1/=-12345.0e0.and.S2/=-12345.0e0) then
      ...
      !主文件
      wln1=nint((S1+swl-B1)/DT)+1  !确定初始时窗:S
      wrn1=nint((S1+swr-B1)/DT)+1  
      !wln11=nint((S1+swl-B1)/DT)+1  !第二初始时窗:S
      ...
      if(srn2>NPTS2) srn2=NPTS2        !判断边界是否超出数据范围
      if(sln2<1) sln2=1
      if(srn2==NPTS2.or.sln2==1) then
