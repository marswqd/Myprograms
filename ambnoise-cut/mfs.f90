...
!作者: 王清东  时间: 2011-09-23 10:14:19
program mfs
 implicit none

 integer*4::i,j,k,lengh,IRU,nerr,num,nt,NPTS
 real*4::snr,gv,ip,it,a,b,alpha,alpha1,alpha2
...
! 读取控制文件信息
 open(11,file='mfsfile',status='old')
 read(11,*) num,dir,nt
 !dir=D:输入的是双端信号
 !dir=-:输入的是非因果信号                  !这里的非因果指的是B<0,E=0的情况
 !dir=+:输入的是因果信号
 allocate(sacname(num),TC(nt))    !nt:滤波中心周期数 alpha:滤波中心周期对应的滤波参数数
 read(11,*) (TC(i),i=1,nt)
 ...
 IRU=100
 a=TC(1)*TC(nt)/(TC(nt)-TC(1))*(sqrt(alpha1)-sqrt(alpha2))
 b=(sqrt(alpha2)*TC(nt)-sqrt(alpha1)*TC(1))/(TC(nt)-TC(1))
...
!  NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 do k=1,num
  ...
  write(10,110)
  110 format(10x'滤波后文件'10x'震中距'2x'中心周期'1x'alpha'2x'信噪比' &
             4x'群速度'1x'瞬时相位(°)'1x'瞬时周期'1x'1:因果 -1:非因果')
  do j=1,nt
   alpha=a/TC(j)+b
   alpha=alpha*alpha
   call brsac(IRU,NPTS,sacname(k),data,nerr) !读取波形数据
! 多重滤波,得到包络和瞬时频率
   call MFT(data,NPTS,rhdr(1),TC(j),alpha,dataE,dataIP)
   wname=sacname(k)
...
! 计算因果信号的信噪比
    ...
    write(10,120) wname(1:lengh+5),rhdr(51),TC(j),alpha,snr,gv,ip,it,' 1'
! 计算非因果信号的信噪比
    call SUF(data,dataE,dataIP,NPTS,TC(j),rhdr(6),rhdr(1),rhdr(51),-1,snr,gv,ip,it,nerr)
    write(10,120) wname(1:lengh+5),rhdr(51),TC(j),alpha,snr,gv,ip,it,'-1'
   else if(dir.eq.'-') then
   ...
   120 format(a,1x,f8.2,2x,f6.2,2x,f6.2,2x,f8.2,2x,f8.4,2x,f6.2,2x,f6.2,2x,a)
! 生成滤波后的文件
   print*, trim(wname)
   call nor(data,NPTS)
...
! dataIP:输出-(瞬时相位,瞬时周期)
 ...
 real*4::dt,TC,fc,df,f,alpha,fac,freqlw,frequp,G,s!omega
