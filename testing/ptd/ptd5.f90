...
program ptd
 ...
 real*4,allocatable::data1(:),data2(:),cor1(:),cor2(:),cor(:),t(:)
...
!读取互相关计算中的副文件(滑动窗)
  ...
  allocate(data2(NPTS2),cor2(pwn2),cor(wrn2-wln2+1),t(wrn2-wln2+1))
  ...
  do j=1,wrn2-wln2+1                           !滑动起来
   ...
   !call nor(cor2,pwn2)
   call td(cor1,pwn1,cor2,pwn2,DT,cc,tdt)  !计算相关函数
   cor(j)=cc
   t(j)=tdt   !相关函数最大值对应时间偏移
   if(cmax.lt.cc) then
    cmax=cc
    k=j
   endif
  ...
  do j=1,wrn2-wln2+1
   write(11,*) (j-(pwln2-wln2+1))*DT+t(j),cor(j)
  enddo
  close(11)
...
SUBROUTINE TD(A,NA,B,NB,DT,CC,TDT)
