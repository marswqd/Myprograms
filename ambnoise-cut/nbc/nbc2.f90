...
!-----
 ...
 integer*4::ntc,pl,ph,vl,vh,tcb,ttb,bbt,interp
 real*4::t0,v0,btc,dtc,etc,dy1,dyn,a,b,alpha1,alpha2,alpha,O
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 do i=1,num
  ...
  if(pl.lt.vl.or.ph.gt.vh) then
   print*, 'The P-window is larger than V-window, please check!'
   goto 99
  endif
...
!读取波形信息并进行窄带滤波
  ...
  do j=1,ntc
   alpha=a/tc(j)+b
   alpha=alpha*alpha
   call MFTC(dataw,VNPTS,rhdr(1),tc(j),alpha)
   temp1=dataw
...
!搜索:走时-周期矩阵;(相速度-周期矩阵)
  ...
  do j=1,ntc
   write(IRU,'(f6.2,2x,f8.3,3f8.4)') TttC(j,:)
  enddo
  close(IRU)
!图示化:周期-速度图
  xy(:,1)=TttC(:,1)
  ...
  deallocate(sT_t,sT_C,tt,wv,tt1,temp1,temp2,TttC,xy)
 enddo
...
subroutine MFTC(data,n,dt,tc,alpha)
! 多重滤波程序(窄带滤波器为高斯滤波器,截断为pai=3.14159265357e0)
! data:输入输出-波形数据
 implicit none
 integer*4::i,n,logn,nfft
 real*4::data(n)
 real*4::dt,tc,fc,df,hf,f,alpha,fac,freqlw,frequp,G,omega
