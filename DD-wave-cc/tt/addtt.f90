!-----------------------------------------------------------------------------!
!--Function: add travel time comes from catalog to the corresponding SAC file
...
!--Creatied time: 2014/4/15 14:17:02
!--Modified time: 2014/4/15 14:17:06
!-----------------------------------------------------------------------------!
program addtt
 implicit none
 integer*4::i,j,m,n,NPTS,num,nerr
 integer*4,allocatable::id(:)
 real*4::T1,T2,DT,ttp,tts
 real*4,allocatable::tt(:),dist(:),az(:)
 character::wname*60,nameZ*60,nameN*60,nameE*60,name1*60,name2*60
 character,allocatable::sta(:)*3,phase(:)*1
...
!读取控制文件信息
 open(11,file='addtt.in',status='old',action='read')
 read(11,*) num
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
 open(12,file='tt-ylall.txt')
 call filelen(12,n)
 allocate(sta(n),tt(n),dist(n),az(n),phase(n),id(n))
 do i=1,n
  read(12,*) sta(i),tt(i),dist(i),az(i),phase(i),id(i)
 enddo
 close(12)
 ...
 do i=1,num
  nameZ=sacname(i)
  j=index(nameZ,'.Z.')
  nameN=nameZ(1:j)//'N.SAC'
  call exist(nameN)
  nameE=nameZ(1:j)//'E.SAC'
  call exist(nameE)
  name1='P.'//nameZ(1:j)//'E.SAC'
  call exist(name1)
  name2='P.'//nameZ(1:j)//'N.SAC'
  call exist(name2)
  m=0
  call brsach(100,nameZ,nerr)
  ttp=rhdr(9)
  tts=rhdr(11)
  !if(trim(chdr(6))=='P'.and.rhdr(9)==-12345.0) then
  if(trim(chdr(6))=='P') then
   do j=1,n
    if(ihdr(13)==id(j).and.trim(chdr(1))==trim(sta(j)).and.phase(j)=='P') then
     m=1
     ttp=tt(j)
     exit
    endif
   enddo
  !elseif(chdr(7)=='S'.and.rhdr(11)==-12345.0) then
  elseif(chdr(7)=='S') then
   do j=1,n
    if(ihdr(13)==id(j).and.trim(chdr(1))==trim(sta(j)).and.phase(j)=='S') then
     m=1
     tts=tt(j)
     exit
    endif
  ...
  if(m==1) then
   rhdr(9)=ttp
   rhdr(11)=tts
   call bwsach(100,nameZ,nerr)
   call brsach(100,nameN,nerr)
   rhdr(9)=ttp
   rhdr(11)=tts
   call bwsach(100,nameN,nerr)
   call brsach(100,nameE,nerr)
   rhdr(9)=ttp
   rhdr(11)=tts
   call bwsach(100,nameE,nerr)
   call brsach(100,name1,nerr)
   rhdr(9)=ttp
   rhdr(11)=tts
   call bwsach(100,name1,nerr)
   call brsach(100,name2,nerr)
   rhdr(9)=ttp
   rhdr(11)=tts
   call bwsach(100,name2,nerr)
   print*, trim(nameZ),'  ',trim(name1)
  endif
 enddo
