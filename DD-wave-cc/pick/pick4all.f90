!本程序的功能是:依据地震目录将地震事件从连续波形记录中截取出来,数据时间范围:发震时刻-震中距
!               并将地震和台站的经纬度,震中距,发震时刻,地震编号,震相信息等写入新sac文件
!  注: 本程序截取全部台站的相应时段波形记录,无论其在地震目录中有没有标定.便于后续的人工标定或理论计算.
!      本程序通过读取sac头文件获取台站名和分量名,不依赖输入文件名,但速度较慢
!            输出sac文件的命名方式为 HWS.20120907111943.E.SAC
!            会剔除数据两端的0值
!作者: 王清东  时间: 2015/8/12 17:01:18
program pick4evt
 implicit none
 
 integer*4::i,j,j1,k,m,n,nn,nm,p,nerr,num,nsta,npos,neve,head,tail,ahead,atail,ID
 integer*4::re_year,re_days,year,month,day,hour,min,nsec,nmsec,days
...
! NZSEC=ihdr(5)  NZMSEC=ihdr(6) NPTS=ihdr(10)  KSTNM=chdr(1)
  ...
  do j1=1,meve
   ...
   enddo
   if(rhdr(9)==-12345.0e0) then   !判断震相附近是否有数据
    chdr(6)='calP'
    p=0
   else
   ...
   !if(rhdr(9)==-12345.0e0) chdr(6)='calP'
   !if(rhdr(11)==-12345.0e0) chdr(7)='calS'
   if(p==0.or.k==0) then
   ...
   if(chdr(6)=='calP'.and.chdr(7)=='calS') then
    write(10,*) trim(wname),dist,depth,mag,ID,'  No'
   else
    write(10,*) trim(wname),dist,depth,mag,ID,'  Yes'
   endif
  enddo
  deallocate(B,E,NPTS,adata)
  write(10,*) 'The station ',trim(sta(i))//'.'//com(i),' should record',neve,'events according to the earthquake catalogue'
  write(10,*) '-----'
  call system('mkdir done')
  command='mv 2*.*.//trim(sta(i))//'.*'//com(i)//'*  done/'
  print*, trim(command)
