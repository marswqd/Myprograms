...
!读取控制文件信息
 ...
 do k=1,stas-1
  do ij=k+1,stas
   ...
   NPTSJ=ihdr(10)
   if(NPTSC.gt.NPTSI.or.NPTSC.gt.NPTSJ) then
...
!计算互相关序列(频率域)
   ...
   if(dir.eq.'-') then
    do l=1,NPTS
     data(l)=dataC(NPTSI+1-l)
    enddo
   else if(dir.eq.'d'.or.dir.eq.'D') then
    do l=1,NPTS
     data(l)=dataC(NPTSI-NPTSC+l)
    enddo
   else if(dir.eq.'+') then
    do l=1,NPTS
     data(l)=dataC(NPTSI+l-1)
    enddo
   else if(dir.eq.'a'.or.dir.eq.'A') then
    data(1)=dataC(NPTSI)
    do l=1,NPTS-1
     data(l+1)=(dataC(NPTSI+l)+dataC(NPTSI-l))/2
    enddo
   endif
!修改必要的SAC头文件变量,生成所需的SAC文件
  ...
  !print*, 'J',KSTNMJ
   if(dir.eq.'d'.or.dir.eq.'D') then
    rhdr(6)=-(NPTSC-1)*rhdr(1)+rhdr(6)-beignI
   else
   ...
   if(dir.eq.'-') then
    ...
    wname(11:11)='D'                       !表示双端信号
   else if(dir.eq.'+') then
    wname(11:11)='+'                       !表示因果信号
   else if(dir.eq.'a'.or.dir.eq.'A') then
    wname(11:11)='A'                       !表示平均信号
   endif
   wname(12:15)='.SAC'
   wname=trim(wname)                        !wname:生成的SAC文件名
   print*, trim(wname)
   call wsac(data,NPTS,wname)               !生成新的SAC文件
   deallocate(dataI,dataJ,dataC,dcr,dci,cr,ci)
  enddo
 enddo

 99 continue
 deallocate(sacname,data)
 close(10)
 call CPU_TIME(T2)
 print*, 'T=',T2-T1
end program
