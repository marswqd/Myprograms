...
!滤波+按震相对其+滑动窗计算相关系数+再次对其+时频域计算时差
!时差为正表示数据2的走时大
!作者: 王清东 时间: 2012-10-08 22:07:11
program ptd
 implicit none
 ...
 real*4::odt,cdt,tdt,fdt,pdt,cmax,cc
...
!读取互相关计算中的副文件(滑动窗)
  ...
  close(11)
  print*, ' '
  call td2(cor1,pwn1,cor2,pwn2,tdf1,tdf2,DT,tdt,fdt)
  ...
  print*, 'odt= ',odt
  print*, 'cdt= ',cdt
  print*, 'tdt= ',tdt,' fdt= ',fdt
  print*, 'pdt1= ',odt+cdt+tdt
  print*, 'pdt2= ',odt+cdt+fdt

  print*, ' '
  print*, 'No shift:'
  cor2(1:pwn2)=data2(pwln2:pwrn1)
  !call nor(cor2,pwn2)
  open(11,file='DATANS.TXT')
  do j=1,pwn1
   write(11,*) cor1(j),cor2(j)
  enddo
  close(11)
...
SUBROUTINE TD2(A,NA,B,NB,F1,F2,DT,TDT,FDT)
...
!CAMP(NFFT),CPHASE(NFFT),MSC(NFFT):互相关谱,互相关相位,平方相干值
 ...
 REAL*4::DT,TDT,DF,FDT,F1,F2,AA,BB,CC,RMSEY,RMSE1,RMSE2,DT1
 REAL*4::PHT,PHF
 REAL*4::A(NA),B(NB),C(-NA+1:NB-1),K(2),CA(-NA+1:NA-1),CB(-NB+1:NB-1)
 REAL*4,ALLOCATABLE::PR(:),PI(:),CR(:),CI(:),AR(:),AI(:),BR(:),BI(:)
 REAL*4,ALLOCATABLE::CAMP(:),CPHASE(:),AAMP(:),BAMP(:),MSC(:),FREQ(:),X(:),Y(:)
 !CALL NOR(A,NA)
 !CALL NOR(B,NB)
...
!频域算法
 ...
 PRINT*, 'PHT= ',PHT
 C=C*SQRT(AA*BB)
 CALL TCOR(A,NA,A,NA,CA,-NA+1,NA-1)
 CALL TCOR(B,NB,B,NB,CB,-NB+1,NB-1)
 CALL NPOW2(NA+NB-1,NFFT,LOGN)
 ALLOCATE(PR(NFFT),PI(NFFT),CR(NFFT),CI(NFFT),AR(NFFT),AI(NFFT),BR(NFFT),BI(NFFT))
 ALLOCATE(CAMP(NFFT),CPHASE(NFFT),AAMP(NFFT),BAMP(NFFT),MSC(NFFT),FREQ(NFFT))
 PR=0.0E0
 PI=0.0E0
 CR=0.0E0
 CI=0.0E0
 !PR(1:NA+NB-1)=C(-NA+1:NB-1)
 PR(1:NB)=C(0:NB-1)
 PR(NFFT-NA+2:NFFT)=C(-NA+1:-1)
 !CALL TAPER(PR,NFFT,0.25)
 CALL KKFFT(PR,PI,NFFT,LOGN,CR,CI,0,0)
 PR=0.0E0
 PI=0.0E0
 AR=0.0E0
 AI=0.0E0
 !PR(1:NA+NA-1)=CA(-NA+1:NA-1)
 PR(1:NA)=CA(0:NA-1)
 PR(NFFT-NA+2:NFFT)=CA(-NA+1:-1)
 !CALL TAPER(PR,NFFT,0.25)
 ...
 BR=0.0E0
 BI=0.0E0
 !PR(1:NB+NB-1)=CB(-NB+1:NB-1)
 PR(1:NB)=CB(0:NB-1)
 PR(NFFT-NB+2:NFFT)=CB(-NB+1:-1)
 !CALL TAPER(PR,NFFT,0.25)
 CALL KKFFT(PR,PI,NFFT,LOGN,BR,BI,0,0)
 DO I=1,NFFT
  CPHASE(I)=ATAN2(CI(I),CR(I))  ![-PI,PI]
  !IF(CPHASE(I).LT.0.0E0) CPHASE(I)=CPHASE(I)+6.283185306E0
  FREQ(I)=(I-1)/(NFFT*DT)
 ENDDO
 CAMP=SQRT(CR*CR+CI*CI)
 AAMP=SQRT(AR*AR+AI*AI)
 BAMP=SQRT(BR*BR+BI*BI)
 CALL SMOOTH(CAMP,NFFT)  !谱平滑
 CALL SMOOTH(AAMP,NFFT)
 CALL SMOOTH(BAMP,NFFT)
 !CALL SMOOTH(CPHASE,NFFT)
 MSC=CAMP*CAMP/(AAMP*BAMP)
 J=0
 N=0
 DO I=1,NFFT
 ...
 !IF(J*1.0E0/N.GT.0.5E0) THEN
  ...
  DO I=1,NFFT
   IF(FREQ(I).GT.F1.AND.FREQ(I).LT.F2) THEN
    J=J+1
    X(J)=FREQ(I)
    Y(J)=CPHASE(I)!*MSC(I)*MSC(I)/abs(1-MSC(I)*MSC(I))
   ENDIF
  ENDDO
  CALL LSLF(X,Y,N,K,CC,RMSEY,RMSE1,RMSE2)
  FDT=-K(2)/6.283185306E0
  PRINT*, 'KL: ',K(1),K(2),'CCXY: ',CC
  PRINT*, 'RMSEY,RMSE1,RMSE2: ',RMSEY,RMSE1,RMSE2
  PRINT*, 'RMSEftd= ',RMSE2/6.283185306E0
 !ENDIF
 OPEN(11,FILE='MSC.TXT')
 DO I=1,NFFT
  WRITE(11,*) FREQ(I),CAMP(I),CPHASE(I),MSC(I),CR(I),CI(I)
 ENDDO
 CLOSE(11)
