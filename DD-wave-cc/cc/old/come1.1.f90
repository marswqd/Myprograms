...
!      滤波+按震相对齐+双滑动窗时域多重互相关检测器(detector)计算时差(双滑动时窗宽度:1s and 2s,单滑动量:1s)
!      correlation detector: Schaff-2005-Waveform Cross-Correlation-Based Differential Travel-Time Measurements
...
!读取控制文件信息
 ...
 open(12,file='come1.1.txt')   !初始互相关走时差文件
    ...
    do jj=1,sta(j)
     ...
     if(name1(mm+1:nn-1)==name2(mm+1:nn-1).and.name1(kk+1:kk+1)==name2(kk+1:kk+1)) then
      ...
      if(iphase/=2.and.P1/=-12345.0e0.and.P2/=-12345.0e0) then
       ...
       do while(k1+wnn-1<=srn11)
        ...
        do while(k2+wnn-1<=srn22)                  !滑动起来
         ...
         bb=0.0e0
         do i1=1,wn
          aa=aa+cr11(i1)*cr22(i1)
         enddo
         do i1=1,wn
          bb=bb+cz11(i1)*cz22(i1)
         enddo
         cc=aa+bb
         aa=0.0e0
         bb=0.0e0
       ...
       cdt2=sdt2-sdt1              !滑动改正量,相关改正量
       pdt2=odt+cdt2               !互相关走时差tt1-tt2
       !P1=P1-sdt1
       !P2=P2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'P:',cmax2,odt,sdt1,sdt2,cdt2,pdt2
       else
        write(10,*) 'P:',cmax2,-odt,sdt2,sdt1,-cdt2,-pdt2
       endif
       ...
       deallocate(cr11,ct11,cz11,cr22,ct22,cz22)
      endif
      !计算S波走时差
      if(iphase/=1.and.S1/=-12345.0e0.and.S2/=-12345.0e0) then
       ...
       !S2=S2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'S:',cmax2,odt,sdt1,sdt2,cdt2,pdt2
       else
        write(10,*) 'S:',cmax2,-odt,sdt2,sdt1,-cdt2,-pdt2
       endif
