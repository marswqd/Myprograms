...
!      滤波+按震相对齐+双滑动窗时域多重互相关函数计算时差(双滑动时窗宽度:1s and 2s,单滑动量:1s)
!      co.in中输入Z分量文件名; ZR分量计算P震相走时差,T分量计算S震相走时差;
!      时差为正表示数据1的走时大::tt1-tt2
!作者: 王清东 时间: 2014/1/9 19:09:36
program come
 implicit none
 ...
 real*4::T1,T2,odt,sdt,sdt1,sdt2,cdt1,cdt2,cdt,tdt,pdt,pdt1,pdt2,cmax,cmax1,cmax2
 real*4::cc,ccut,d12,az1,az2,az,alat,alon,dlat,dlon,stla,stlo
...
!读取控制文件信息
 ...
 read(11,*) iphase,pwl,pwr,swl,swr !双滑动时窗宽度设定,震相到时左右偏移量(s)
 ! 一般 pwl=-0.5e0 pwr=0.5e0 swl=-1.0e0 swr=1.0e0
 read(11,*) sld1,sld2  !滑动量,一般sld1=sld2=1.0e0
 ...
 open(12,file='come.txt')   !初始互相关走时差文件
    ...
    do jj=1,sta(j)
     ...
     if(name1(mm+1:nn-1)==name2(mm+1:nn-1).and.name1(kk+1:kk+1)==name2(kk+1:kk+1)) then
      ...
      if(iphase/=2.and.P1/=-12345.0e0.and.P2/=-12345.0e0) then
       ...
       slidel=sld1
       slider=sld1
       if((0.73*(P1-O1)-swr)>0.0e0) then
        slider=min(slider,0.73*(P1-O1)-swr)
       else
        slider=0.0e0
       endif
       !主文件
       sln1=nint((P1+pwl-slidel-B1)/DT)+1  !判断边界是否超出数据范围
       srn1=nint((P1+pwr+slider-B1)/DT)+1  !第一时窗边界
       sln11=nint((P1+swl-slidel-B1)/DT)+1  !判断边界是否超出数据范围
       srn11=nint((P1+swr+slider-B1)/DT)+1  !第二时窗边界
       ...
       wln1=nint((P1+pwl-B1)/DT)+1  !确定固定窗:P
       wrn1=nint((P1+pwr-B1)/DT)+1  !第一P波窗
       ...
       slidel=sld2
       slider=sld2
       if((0.73*(P2-O2)-swr)>0.0e0) then
        slider=min(slider,0.73*(P2-O2)-swr)
       else
        slider=0.0e0
       endif
       sln2=nint((P2+pwl-slidel-B2)/DT)+1  !判断边界是否超出数据范围
       srn2=nint((P2+pwr+slider-B2)/DT)+1  !第一时窗边界
       sln22=nint((P2+swl-slidel-B2)/DT)+1  !判断边界是否超出数据范围
       srn22=nint((P2+swr+slider-B2)/DT)+1  !第二时窗边界
       ...
       sdt=sdt2-sdt1               !滑动改正量
       cdt1=sdt+tdt                !相关改正量
       pdt1=odt+cdt1               !互相关走时差tt1-tt2
       !P1=P1-sdt1
       !P2=P2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'P:',cmax1,odt,sdt1,sdt2,tdt,cdt1,pdt1
       else
        write(10,*) 'P:',cmax1,-odt,sdt2,sdt1,-tdt,-cdt1,-pdt1
       endif

       !第二窗宽
       cmax2=-99999.0e0
       k1=sln11
       do while(k1+wnn-1<=srn11)
        cr11(1:wnn)=dr1(k1:k1+wnn-1)
        !ct11(1:wnn)=dt1(k1:k1+wnn-1)
        cz11(1:wnn)=dz1(k1:k1+wnn-1)
        call nor(cr11,wnn)
        !call nor(ct11,wnn)
        call nor(cz11,wnn)
        k2=sln22
        do while(k2+wnn-1<=srn22)                  !滑动起来
         cr22(1:wnn)=dr2(k2:k2+wnn-1)
         !ct22(1:wnn)=dt2(k2:k2+wnn-1)
         cz22(1:wnn)=dz2(k2:k2+wnn-1)
         call nor(cr22,wnn)
         !call nor(ct22,wnn)
         call nor(cz22,wnn)
         call tcor(cr11,wnn,cr22,wnn,cr1122,-wnn+1,wnn-1)  !时域中计算互相关函数
         aa=0.0e0
         bb=0.0e0
         do i1=1,wnn
          aa=aa+cr11(i1)*cr11(i1)
         enddo
         do i1=1,wnn
          bb=bb+cr22(i1)*cr22(i1)
         enddo
         cr=sqrt(aa*bb)
         call tcor(cz11,wnn,cz22,wnn,cz1122,-wnn+1,wnn-1)  !时域中计算互相关函数
         aa=0.0e0
         bb=0.0e0
         do i1=1,wnn
          aa=aa+cz11(i1)*cz11(i1)
         enddo
         do i1=1,wnn
          bb=bb+cz22(i1)*cz22(i1)
         enddo
         cz=sqrt(aa*bb)
         c1122=(cr1122+cz1122)/(cz+cr)
         cc=0.0e0
         j1=0
         do i1=-wnn+1,wnn-1
          if(cc.lt.c1122(i1)) then
           cc=c1122(i1)
           j1=i1
          endif
         enddo
       ...
       odt=(P1-O1)-(P2-O2)         !初始定位的震相走时差
       sdt1=(wln11-cn1)*DT
       sdt2=(wln22-cn2)*DT         !移动滑动窗产生的时间差,左移为正,右移为负
       sdt=sdt2-sdt1               !滑动改正量
       cdt2=sdt+tdt                !相关改正量
       ...
       !P2=P2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'P:',cmax2,odt,sdt1,sdt2,tdt,cdt2,pdt2
       else
        write(10,*) 'P:',cmax2,-odt,sdt2,sdt1,-tdt,-cdt2,-pdt2
       endif
      ...
      if(iphase/=1.and.S1/=-12345.0e0.and.S2/=-12345.0e0) then
       ...
       if((0.73*(P1-O1)+swl)>0.0e0) then
        slidel=min(slidel,0.73*(P1-O1)+swl)
       else
        slidel=0.0e0
       endif
       !主文件
       sln1=nint((S1+pwl-slidel-B1)/DT)+1  !判断边界是否超出数据范围
       srn1=nint((S1+pwr+slider-B1)/DT)+1  !第一时窗边界
       sln11=nint((S1+swl-slidel-B1)/DT)+1  !判断边界是否超出数据范围
       srn11=nint((S1+swr+slider-B1)/DT)+1  !第二时窗边界
       if(srn1>NPTS1) srn1=NPTS1
       if(sln1<1) sln1=1
       if(srn1==NPTS1.or.sln1==1) then
       ...
       wln1=nint((S1+pwl-B1)/DT)+1  !确定固定窗:S
       wrn1=nint((S1+pwr-B1)/DT)+1  !第一S波窗
       ...
       if((0.73*(P2-O2)+swl)>0.0e0) then
        slidel=min(slidel,0.73*(P2-O2)+swl)
       else
        slidel=0.0e0
       endif
       sln2=nint((S2+pwl-slidel-B2)/DT)+1  !判断边界是否超出数据范围
       srn2=nint((S2+pwr+slider-B2)/DT)+1  !第一时窗边界
       sln22=nint((S2+swl-slidel-B2)/DT)+1  !判断边界是否超出数据范围
       srn22=nint((S2+swr+slider-B2)/DT)+1  !第二时窗边界
       if(srn2>NPTS2) srn2=NPTS2
       if(sln2<1) sln2=1
       if(srn2==NPTS2.or.sln2==1) then
       ...
       !S2=S2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'S:',cmax1,odt,sdt1,sdt2,tdt,cdt1,pdt1
       else
        write(10,*) 'S:',cmax1,-odt,sdt2,sdt1,-tdt,-cdt1,-pdt1
       endif
       ...
       do while(k1+wnn-1<=srn11)
        ...
        do while(k2+wnn-1<=srn22)                  !滑动起来
         ...
         !enddo
         !cz=sqrt(aa*bb)
         call tcor(ct11,wnn,ct22,wnn,ct1122,-wnn+1,wnn-1)  !时域中计算互相关函数
         aa=0.0e0
         bb=0.0e0
         do i1=1,wnn
          aa=aa+ct11(i1)*ct11(i1)
         enddo
         do i1=1,wnn
          bb=bb+ct22(i1)*ct22(i1)
         enddo
         ct=sqrt(aa*bb)
         !c1122=(cr1122+cz1122+ct1122)/(cr+cz+ct)
         c1122=ct1122/ct
         cc=0.0e0
         j1=0
         do i1=-wnn+1,wnn-1
       ...
       !S2=S2-sdt2-tdt
       if(ID1<ID2) then
        write(10,*) 'S:',cmax2,odt,sdt1,sdt2,tdt,cdt2,pdt2
       else
        write(10,*) 'S:',cmax2,-odt,sdt2,sdt1,-tdt,-cdt2,-pdt2
       endif
