...
!-----
 ...
 real*4::t0,v0,btp,dtp,etp,TC,nbm,nbn
 real*4::B,O,DT,DIST
 real*4,allocatable::tp(:),saccut(:),tt(:),wv(:)
 real*4,allocatable::PR(:),PI(:),FR(:),FI(:),RR(:),II(:),nb(:)
 real*4,allocatable::sacdata(:,:),sacpv(:,:),pv(:,:) 
 real*4,parameter::vs=2.0,ve=5.0
...
!读取控制文件信息 
 open(10,file='bngfile',status='old')
 read(10,*) t0,v0
 read(10,*) sacname
...
!读取SAC文件并剪取信息到saccut
 ...
 i=floor(abs((DIST/vs-B)/DT))+1
 j=floor(abs((DIST/ve-B)/DT))
!vs<ve:i>j
 NPTS=abs(i-j+1)
 B=B+(j-1)*DT
...
!序列补零+FFT+nbf+IFFT,保存滤波信息到sacdata 
 ...
 do i=1,ntp
  ...
  if(TC.lt.60.0) then
   nbm=1.0
   nbn=0.1
  elseif(TC.lt.80.0) then
   nbm=1.94
   nbn=0.13
  elseif(TC.lt.120.0) then
   nbm=2.88
   nbn=0.3
  else
   nbm=2.5
